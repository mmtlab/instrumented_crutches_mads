/*
 ...
 |_|   |_|_\__\___|_|    | .__/|_|\__,_|\__, |_|_| |_|
                          |_|            |___/         
# A Template for Error_handlerPlugin, a Filter Plugin
# Generated by the command: C:\Program Files\MADS\usr\local\bin\mads-plugin.exe -t filter -d C:\mirrorworld\instrumented_crutches\error_handler error_handler
# Hostname: unknown
# Current working directory: C:\mirrorworld\instrumented_crutches\error_handler
...
#ifndef PLUGIN_NAME
#define PLUGIN_NAME "error_handler"
#endif
...
class Error_handlerPlugin : public Filter<json, json> {
...
public:
  ...
  return_type load_data(json const &input, string topic = "") override {
    ...
    if (input.contains("settings") && input["settings"].is_object()) {
      try {
    ...
    if (input.contains("offset_left") || input.contains("offset_right") || 
      ...
      if (_debug) {
        std::cout << "ErrorHandler: offset detected topic='" << topic << "' message='" << message << "'" << std::endl;
      }
    }
  ...
  return_type process(json &out) override {
    ...
    if (_pending.empty()) {
      if (_debug) {
        std::cout << "ErrorHandler: process retry (no pending messages)" << std::endl;
      }
    ...
    if (_debug) {
      std::cout << "ErrorHandler: emitting status=" << out.dump(4) << std::endl;
    }
...
*/
INSTALL_FILTER_DRIVER(Error_handlerPlugin, json, json);
